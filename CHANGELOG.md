...
- **Web 输入历史 + 导出对话**（`docs/0.2.8/PLAN-N15-WEB-FEATURES.md`，N15 2/5）:
  ...
  - **导出对话**: `GET /api/session/:id/export` 全量会话 JSON（`{name,model,exported_at,messages[]}`，复用 `writeMessagesRange`+`formatMessageJson` 与消息获取字节一致，name/model 走 `escapeAlloc`）；侧边栏 more-menu 新增 Export 按钮（两处渲染点），前端 `JSON.parse`+`JSON.stringify(_, null, 2)` pretty 后 Blob + `<a download>` 触发下载（服务器序列化器保持紧凑不动，格式化在浏览器 JSON 引擎做，人读友好且零后端成本），下载文件名 `sessionExportFileName(s)`（sanitize 会话名：非法字符替换 `_`、去首尾点空格、截断 100 字符；uuid/空名回退 `session-<id>.json`，新增 `tests/frontend/test-export-name.mjs` 15 断言），失败 `showStatus` 提示。实机验证: 57 条真实会话导出 JSON 合法 + 与 `GET /api/session/:id` 逐消息字节一致
...
- **工具卡片类型化渲染**（`docs/0.2.8/PLAN-TOOL-CARD-TYPED.md`，11 前端测试文件）:
  - `ToolRegistry` 纯函数化（幂等可重放，对齐 dsh presentCall/presentResult 理念）：webfetch 视图（url/format/mime/字节数）+ edit diff 高亮（+/- 行着色）+ bash pre/code 幂等包装 + `fallback` 兜底渲染器（未知工具确定性走通用卡片）
  - **服务端 ToolMeta 全字段持久化**：`Message.meta` 挂 `role=tool` 消息（agent.zig append 处是数据源），session serialize/parse/append 三处 + handler 透出到 Web API——reload 后工具卡片与流式一致（此前 reload 全丢 meta）
  - 幂等契约三层：bash pre/code 判重、setToolMeta 节点复用、parts 从累积 `_toolData` 构建（多段 tool_meta 不覆盖丢失）
...
- **glob 返回路径带 `.` 前缀**（用户实测，`docs/0.2.8/PLAN-TOOL-TRAVERSE.md`）: `glob("**/*")` 默认 `path` 下返回 `.Documents/test/demo-app.py`、`.AGENTS.md` 等——真实路径无 `.` 前缀，按返回路径 read/grep 直接 FileNotFound。根因: `path="."`（默认值）时 `prefix="."` 命中直拼分支 → `"." + "Documents" = ".Documents"`，递归层叠后全链路带点。修复: 顶层拼接对 `""`/`"."` 前缀直接输出 `entry.name`（无前缀）。新增回归测试 `glob: dot path yields unprefixed top-level names`（断言无 `.a.zig`/`.sub/b.zig`/`./sub/b.zig`）
- **grep 目录搜索不递归子目录**（用户实测，`docs/0.2.8/PLAN-TOOL-TRAVERSE.md`）: `grep` 对目录搜索只扫顶层文件，子目录命中漏报为 "No matches"（含 include 过滤场景），误导 Agent 误判无匹配。根因: `searchDir` 循环 `entry.kind != .file` 直接 continue，无递归逻辑。修复: 拆分 `searchDir`（外层编译 Pattern + 收尾统计）+ 新增 `walkDir` 递归（`rel_prefix` 相对路径输出 `sub/nested.txt`、`truncated` 兼作跨层停止标志、目录 openDir 失败静默跳过）。新增集成测试 `grep: directory search recurses into subdirs`（两级子目录命中 + 顶层无匹配文件不误报）
...
- **工具卡片 card-body 嵌套致渲染错位**（用户实测）: CARD-UNIFY 改造后 buildSegment 的 content/output 元素误设 `card-body` 类，makeCard 又包一层 `.card-body` → 双重嵌套，CSS 骨架 `.card > .card-body` 只匹配直接子级致显隐失效 + 输出错位。修复: body 元素只带特化类（`content`/`output`），card-body 由 makeCard 负责包装。新增 test-build-segment 无嵌套断言
