...
- **摩擦自愈机制**（审查修订）：分级之外，双机制降低重复摩擦：
  ...
  - **配置白名单**（审查修订：原"字符串含匹配"过宽——`"rm -rf .zig-cache"` 会豁免 `rm -rf .zig-cache-extra`（前缀家族）和 `rm -rf .zig-cache && rm -rf /`（尾追加，事故/蓄意均能绕过），无词边界；改为**归一化整命令精确匹配**）：`approval_allow = ["rm -rf .zig-cache", "git push --force origin dev"]`——`args.command` 归一化后（**trim + 内部空白折叠为单空格 + 大小写不敏感**，引号不做等价归并）与条目**完全相等**才豁免。命中：`rm -rf .zig-cache`/`rm -rf  .zig-cache`（空白折叠）/`RM -RF .ZIG-CACHE`；**不命中**：`rm -rf .zig-cache-extra`/`rm -rf .zig-cache && rm -rf /`/`rm -rf .zig-cache/`（尾斜杠为不同命令，用户可配两条）
