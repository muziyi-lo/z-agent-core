...
| N23 | **配置检查功能补全**— **P2** | **方案已定稿（2026-08-16，docs/0.2.8/PLAN-N23-CONFIG-CHECK.md）**：对齐 opencode 配置错误可见性（parse.ts 行列定位 + Schema 全错误）。三档：①**default_model 有效性校验**（validateConfig 补 resolveModel——配错模型启动即报错 fail-fast，新错误码 InvalidConfig_DefaultModelUnresolvable）②**标量字段类型警告**（warnWrongType——default_model=123 等静默回退场景显式警告，覆盖 8 个顶层标量键）③**TOML 语法行列定位**（toml.zig parse 内行号跟踪 + stderr 打印行号/行内容，error.InvalidToml 码不变）。明确排除：结构化错误消费、未知键 fail-fast、F25 嵌套表 |
