...
| N16 | 工具审批 + 文件/图片预览（PARTS"高价值三件"剩余，2026-08-13 审计发现）— **P1** | **✅ 已实施（2026-08-16，PLAN-N16-APPROVAL-PREVIEW）**：审批=`src/approval.zig`（Gate 原子状态机 + isRisky L0/L1 两级规则集 + messageFor 三态措辞 + policyNotice 策略段；L0 动态执行 fail-closed，L1 递归删除/存储破坏/系统破坏/git 破坏/管道执行封闭清单；`approval_mode` never/risky/always 默认 risky + `approval_allow` 归一化精确白名单 + `approval_cache` 回合缓存）；Web=`approval_required/reminder` SSE + ApprovalModal（deadline 显示/排队上限 2/stale 竞态）+ `POST /api/approval/:id`（UUID 不可预测 + session 绑定 404）；CLI=fail-closed 确定性拒绝（no_interactive）；预览=`GET /api/preview`（JSON 四态 image/text/binary/too_large + raw=1 原始字节仅图片白名单 + CSP `img-src 'self'` + 413）；server 非回环绑定无鉴权拒绝启动。**L1 规则集审计登记**：活清单随版本扩充（对齐 F14 审查模式） |
...
| N20 | **grep 正则支持** | **✅ 已实施（2026-08-15，docs/0.2.8/PLAN-GREP-REGEX.md）**：新增 `util/regex.zig` 轻量正则引擎（next 链 AST + 非锚定搜索 + 字面量前缀驱动 + 失败记忆化 + 按输入规模缩放步数预算 + 迭代顺行链/重复零递归 + 编译期深度/量词上限）；grep 接入（pattern 恒为正则，`errorDetail` 修复指引 + pos 定位）；25 引擎单测 + 3 集成测试（含语义回归：非锚定/预算熔断/长链零递归）。语义变化：含特殊字符模式从子串变为正则（纯字面量不变）。替代 `std.regex`（0.16 移除）方案定案：自实现子集（非完整 ECMA） |
...
| N22 | **多模态图片附件（vision）**— **P1** | **✅ 已实施（2026-08-16，docs/0.2.8/PLAN-N22-VISION-ATTACHMENTS.md）**：附件链路全通——types.Attachment{ToolResult/Message 双挂载}→ read magic 嗅探+尺寸守卫(短边>2000 拒)+5MB 上限+base64 附件 → session serialize/parse/dup(null 省略) → provider `input_modality` 门控注入 image_url(校验: mime 白名单/base64 容忍行折叠/累计 5MB) → agent 临时 Notice(非 vision, 不写 session)。偏差：CLI 无附件内联渲染(工具摘要文本已覆盖, ToolMeta 无附件字段 YAGNI)。当前 DeepSeek V4 text-only 配置下 read 图片=摘要+Notice，配置 `input=["text","image"]` 激活视觉(未知枚举值有警告)。375 tests |
| F23 | **图片缩放模块（vision 成本控制）**— **P2，依赖 N22** | **登记（2026-08-16，对齐 opencode photon.ts 源码验证）**：opencode `core/src/image/photon.ts`——超限时 32 档递减尺寸（×0.75/Lanczos3）× PNG+JPEG[80,85,70,55,40] 候选编码，首个 ≤max_base64_bytes 返回，全败 SizeError；配置 `attachments.image{auto_resize,max_width,max_height,max_base64_bytes}`（默认 2000/2000/5MB，auto_resize 默认 true）。动机：vision 按图块计费，原样大图烧 token。Zig 方案：vendored stb_image/stb_image_resize2/stb_image_write 纯 C 库，策略对齐 photon.ts；N22 已用纯头部解析拦截短边 >2000px（第一层），本项为第二层（1000-2000px 中等图降本） |
| N23 | **配置检查功能补全**— **P2** | **✅ 已实施（2026-08-16，docs/0.2.8/PLAN-N23-CONFIG-CHECK.md）**：三档配置错误可见 + 退出码配套。①**default_model 有效性**（error，校验点=init.zig:85 启动必经路径；`--model` 覆盖前移至 init opts——校验用覆盖后值无误报；无新增错误码，复用 ModelResolveFailed；错误文本含 formatAvailableModels 建议；web 新增 listen 前校验点）②**标量字段类型警告**（warnWrongType 汇总块，覆盖 8 个顶层标量键，回退默认）③**TOML 语法行列定位**（toml.zig `Diag` out-param 诊断 + config.zig 单点渲染行列+caret，parse 保持纯净无 stderr）。配套：配置/init 错误路径退出码 0→1。383 tests。明确排除：结构化错误消费、未知键 fail-fast、F25 嵌套表、收集全部 TOML 语法错误（error-tolerant 解析器重构） |
| F25 | **TOML 嵌套表支持**（`[models.compat]` 平铺陷阱）— **P3** | **登记（2026-08-16，真实事故）**：轻量 toml.zig 把 `[models.compat]` 后键平铺到 `root["models.compat"]`——模型表的 `input`/`thinking_format` 等键**静默丢失**（input 回退 ["text"]、compat override 永不生效）。已加启动警告（检测到 `models.compat` 键即 stderr 提示）+ 模板注释改为纯注释（移除真实表头）。方向：toml.zig 支持嵌套表（`[a.b]` 归入 `a` 表）或对嵌套表头直接报错（fail-fast）；需评估与现有 config 兼容 |
| F24 | **附件外置存储/会话清理（vision 会话膨胀）**— **P3，依赖 N22** | **登记（2026-08-16，评论提示 + 方案细化 + API 验证）**：多图内联 base64（≤5MB/张 → base64 ~6.7MB）使会话文件快速增长。两个候选方案（评估后选一或组合）：**①压缩 base64 存储**——诚实评估：PNG/JPEG/WEBP 已压缩，对解码字节压缩收益极低；但 **base64 文本有 ~25% 熵冗余**，可省 ~15-25%。**API 验证（2026-08-16 桩编译）**：`std.compress.gzip` 不存在、`std.compress.zstd` 仅解压（无 Compress 导出）——可用 `std.compress.flate.Compress`（deflate）+ 手工 gzip/zlib 头尾（~30 行）实现，可行**②文件引用外置**（`session_assets/{id}.bin` 存原始字节，会话 JSON 存引用 + 按需加载）——零膨胀真方案，但需处理与 fork/branch（复制资产）/export（导出资产或降级引用）/undo（引用回滚）/会话删除（级联清理资产）的关系，复杂度高。当前内联对齐 opencode 全量落盘，可接受 |
...
| F6 | **dsh 架构借鉴候选**（DeepSeek Harness 对比调研 2026-08-13，非 0.2.8 主题）— **P3 长期** | 因 dsh 突然发布而做的对比研究（文档：`deepseek-harness-vs-zagent-core.md`，位于仓库外上层 docs/ 目录，不占发布周期）。低成本可采纳（按成本/收益排序）：① **request/header 请求快照**——每次请求把"系统提示词+工具 schema+请求配置"落盘为只读记录，获请求级审计/调试（无需完整事件溯源）；② **chunk 级持久化**——SSE 原始 chunk 追加进会话（trace 级即可），崩溃后 UI 可重放；③ **compaction 影子记录**——压缩时记录 shadowedRange/shadowedSeqs（哪怕仅日志），保留可审计性。长期：fail-closed 审批（unavailable=拒绝）、单调守卫（只能拒绝不能放行）、per-call 沙箱策略。详见对比文档第 10 节 |
